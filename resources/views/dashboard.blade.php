<x-app-layout>
    ...
    <header class="bg-white shadow">
        <div class="max-w-7xl mx-auto py-2 px-4 sm:px-6 lg:px-8">
            <span class="text-indigo-600 font-semibold text-xl">Dashboard</span>
            @if (auth()->user() && auth()->user()->role_id == 2)
                {{-- <a href="{{ route('jobs.create') }}" class="btn btn-primary" style="float:right;">
                    <i class="fas fa-plus mr-2"></i>Create a new job</a> --}}
            ...
            @if (auth()->user() && auth()->user()->role_id == 1)
                <a href="{{ route('user-detail.index', auth()->user()->id) }}" style="float:right;">
                    <x-jet-button><i class="fas fa-plus mr-2"></i>Make my CV</x-jet-button>
                </a>
            @endif
                ...
                @if (auth()->user()->role_id == 1)
                        ...
                        @foreach (auth()->user()->proposals as $proposal)
                            <div class="mb-3 {{ $proposal->validated ? 'text-green-500' : '' }}">
                                <span class="block  items-center">
                                    ...
                                    for the job <span class="font-semibold">"{{ $proposal->job->title }}"</span>
                    ...
                    <section class="text-sm text-gray-700 w-full w-1/3 mr-5">
                        <h2 class="text-xl my-2"><svg class="w-6 h-6 inline-block" fill="none" viewBox="0 0 24 24"
                            ...
                            </svg> Your favorites jobs ({{ auth()->user()->likes()->count() }})</h2>
                            ...
                            <tbody style="font-weight: 500">
                                ...
                                @foreach (auth()->user()->jobs as $job)
                                    <tr>
                                        ...
                                        @if ($job->status == 1)
                                            <td>
                                                <span class="badge rounded-pill bg-success ml-3"
                                                    style="font-size:11px">Available</span>
                                            </td>
                                        @else
                                            <td>
                                                <span class="badge rounded-pill bg-danger ml-3"
                                                    style="font-size:11px">Unavailable</span>
                                            </td>
                                        @endif
                                        ...
                                        <td>
                                            <a href="{{ route('jobs.edit', $job->id) }}" class="btn"
                                                style="color: green"><i class="far fa-edit"></i></a> | |
                                            <a href="{{ route('jobs.delete', $job->id) }}" class="btn"
                                                style="color: red" onclick="return confirm('Are you sure ?');"><i
                      ...
                      @choice('proposition|propositions', $job->proposals)) :</span>
                                ...
                                <ul class="list-none ml-4">
                                    @foreach ($job->proposals as $proposal)
                                        <li class="mt-2">• Send by <strong>{{ $proposal->user->name }}</strong>
                                            for the job
                        ...
                        @endif --}}
                                        ...
                                        @if ($proposal->validated)
                                            <span
                                                class="bg-white border border-green-500 text-md p-1 my-2 inline-block text-green-500 rounded"
                                                id="deplier">
                                                Already validated</span>

                                            <a href="{{ route('cancel.proposal', $proposal->id) }}"
                                                class="bg-red-300 border border-green-500 text-md p-1 my-2 mb-4 inline-block text-black rounded hover:bg-red-500 hover:text-white duration-500 transition">
                                                Cancel validation</a>

                                            <button type="button" class="bg-green-500 text-white border border-green-500 text-md p-1 my-2 mb-4 inline-block text-black rounded hover:bg-green-300 hover:text-white duration-500 transition" data-mdb-toggle="modal"
                                                data-mdb-target="#appointmentModal">
                                                Appointment schedule <i class="far fa-time ml-3" style="font-size: "></i>
                                            </button>
                                        ...
                                        @else
                                            <a href="{{ route('confirm.proposal', $proposal->id) }}"
                                                class="bg-green-500 text-md py-2 px-2 mt-5  inline-block text-white hover:bg-green-300 hover:text-green-500 duration-200 transition rounded">
                                                Approve proposal</a>
                                            ...
                                            <button type="button" class="btn btn-primary" data-mdb-toggle="modal"
                                                ...
                                                View of cv <i class="far fa-time ml-3" style="font-size: 20px"></i>
                                            </button>
                                                ...
                                                <div class="modal-dialog modal-lg">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-body">
                                                            {{-- {{ dd($proposal->user->detail) }} --}}
                                                            @include('cv.resumeProposal')
                                                            {{-- <iframe src="{{ route('resume-proposal.index') }}"
                                                        ...
                                                        <div class="modal-footer">
                                                            ...
                                                            <a href="{{ route('resume-proposal.download') }}"
