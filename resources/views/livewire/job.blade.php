    <div class="col-md-6 col-sm-12 col-lg-12">
        <div class="px-3 py-3 mb-2 hover:shadow-lg rounded border border-gray-500 bg-gray-200">
            <span class="text-sm text-gray-600 d-flex mb-2">
                
                <img src="{{ Auth::user()->profile_photo_url }}" style="width: 25px; height: 25px"
                class="rounded-circle mr-3" alt="{{ Auth::user()->name }}">
            ...
            <div class="flex justify-between">
                <div class="text-xl font-bold text-indigo-600">{{ $job->title }}
                    @if ($job->status==1)
                        <span class="badge rounded-pill ml-3" style="font-size:11px; background-color:#6875FC;">Available</span>
                    @else
                ...
                <button class="h-6 w-6 text-gray-600 focus:outline-none" wire:click="addLike">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="{{ $job->isLiked() ? 'indigo' : 'white' }}"
            ...
            <span class="ml-5">
                <a href="{{ route('jobs.show', $job->id) }}" 
                    class="text-indigo-500" style="text-decoration: underline;">Show more
                </a>
            </span>
            <div class="text-sm text-gray-600 mt-2 "><strong>{{ $job->number }} places.</strong> Now there is {{ $job->proposals->count() }} @choice('proposal|proposals',
                $job->proposals) for this job
            </div>
